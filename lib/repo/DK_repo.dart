...
class DKRepo {
  Future getListDK() async{
    ...
    try {
      Response res = await callApi(ApiMethods.GET, '/dk/list/'+user.branch!.branchId);
  ...
  Future createDK(Map<String,dynamic> form_data) async{
    try {
      Response res = await callApi(ApiMethods.POST, '/dk/create',data: form_data);
      log(json.encode(res.data));
  ...
  Future approvalDK(String id_dk,Map<String,dynamic> form_data) async{
    try {
      Response res = await callApi(ApiMethods.POST, '/dk/approved/'+id_dk,data: form_data);
      log(json.encode(res.data));
      return true;
    } catch (e) {
