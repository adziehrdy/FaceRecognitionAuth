...
class GlobalRepo {
  ...
  try {
    Response res = await callApi(ApiMethods.GET, '/auth/getversion', data: {});
    if (res.statusCode == 200) {
      String latestVersion = ""; // Inisialisasi variabel latestVersion sebagai String
      ...
      latestVersion = appversionModel.data.multiple.firstWhere(
        (item) => item.os == platform,
      ).version;
      
      SharedPreferences prefs = await SharedPreferences.getInstance();
      await prefs.setString("LATEST_VERSION", latestVersion); // Simpan nilai latestVersion
  ...
  Future<bool> hitUpdateLokasi(String latlong, String Alamat) async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        showToast("Mohon Cek Koneksi Internet | "+(res.statusCode?? "").toString());
       return false;
      }
    } on DioError catch (e) {
      print("Dio error: $e");
