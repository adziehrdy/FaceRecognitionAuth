...
class AttendanceRepos {
  Future<Response> afterScanCheck(Map<String, dynamic> map, String type) async {
    return await callApi(ApiMethods.POST, '/attendance/after-scan-check/$type',
        data: map);
  }
  ...
  Future uploadAbsensi(Attendance attendance, String id) async {
    ...
    Map<String, dynamic> data = {
      "attendance": attendance.toCreateMapForHitAPI(info.version),
    ...
    return await callApi(ApiMethods.POST, '/attendance/demo/wfoinput',
        data: data);
  }

  Future verifyAbsensiWFH(File image, Attendance attendance,
      AttendanceApproval? attendanceApproval, String? mode) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    // String? token = prefs.getString("TOKEN");
    String? token =
        "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..2UOpslKN-YUMWUYIfpP_1vMIWVaQerDzv5h21DEeQjg";
    ...
    try {
      Uri url = Uri.parse(
          endpointUrl! + '/attendance/demo/wfhinput/$mode?token=$token');
      ...
      if (attendanceApproval != null) {
        req.fields["attendanceApproval"] =
            json.encode(attendanceApproval.toMap());
      }
  ...
  Future<Map<String, dynamic>> canCheckIn(String id) async {
    ...
    try {
      Response res = await callApi(
          ApiMethods.POST, '/attendance/demo/can-checkin',
          data: {"employee_id": id});
      if (res.statusCode == 200) {
        return {
          "canCheckIn": res.data['canCheckIn'],
          "lastAttendance": res.data['lastAttendance'] == null
              ? null
              : Attendance.fromMap(res.data['lastAttendance'])
        };
      }
    } catch (e) {
      SharedPreferences pref = await SharedPreferences.getInstance();
      bool b = pref.getBool('CANCHECKIN') ?? true;
      return {"canCheckIn": b, "lastAttendance": null};
    }

    // Add a return statement here to handle the case when none of the conditions above are met
    return {"canCheckIn": false, "lastAttendance": null};
  }
  ...
  Future<Map<String, dynamic>> canCheckOut(String pageType, String id) async {
    ...
    try {
      Response res = await callApi(
          ApiMethods.POST, '/attendance/demo/can-checkout/$pageType',
          data: {"employee_id": id});
      if (res.statusCode == 200) {
        return {
      ...
      return {"canCheckOut": b, "lastAttendance": null};
    }

    // Add a return statement here to handle the case when none of the conditions above are met
    return {"canCheckOut": false, "lastAttendance": null};
  }

  List<Map<String, dynamic>> checkGeofencing(
      double? lat, double? lng, List<Branch> branches) {
    final Distance distance = new Distance();
    List<Map<String, dynamic>> map = [];
    for (Branch branch in branches) {
      final meter = distance.as(LengthUnit.Meter, LatLng(lat!, lng!),
          LatLng(branch.branchLat, branch.branchLng));
      map.add({
        "branch": branch,
  ...
  Future<Map<String, dynamic>?> getAttendanceList(String? month, int page,
      List<String?>? absenStatus, List<String>? ijinStatus) async {
    try {
      Response res = await callApi(
          ApiMethods.POST, '/attendance/list-kehadiran/$month?page=$page',
          data: {"absenStatus": absenStatus, "ijinStatus": ijinStatus});
      return res.data;
    } catch (e) {
      throw e;
    }
  ...
  Future<Map<String, dynamic>?> getAttendanceListDateRange(
      String start,
      String end,
      int page,
      List<String?>? absenStatus,
      List<String>? ijinStatus) async {
    try {
      Response res = await callApi(
          ApiMethods.POST, '/attendance/list-kehadiran/$start/$end?page=$page',
          data: {"absenStatus": absenStatus, "ijinStatus": ijinStatus});
      return res.data;
    } catch (e) {
      throw e;
